...
use bevy::{
    ...
    log::{self, error},
    prelude::{Commands, Entity, EventReader, NonSend, Query, Resource, Time, Window, With},
    time::Real,
...
pub fn process_output_system(
    ...
    for mut context in contexts.iter_mut() {
        ...
        if let Some(mut _window) = context.window {
            let mut set_icon = || {
                for window in windows.iter() {
                    commands
                        .entity(window)
                        .insert(bevy::winit::cursor::CursorIcon::from(
